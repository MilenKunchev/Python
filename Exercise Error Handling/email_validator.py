...
valid_domains_string = ''
for domain in valid_domains:
    valid_domains_string += f'.{domain}, '
...
while True:
    ...
    if len(domain_name_parts) < 2 or not all([len(x) > 1 for x in domain_name_parts]):
        raise InvalidDomainNameError('Invalid domain name')

    domain = domain_name_parts[-1]
    if domain not in valid_domains:
        raise InvalidDomainError(f'Domain must be one of the following: {valid_domains_string}')
